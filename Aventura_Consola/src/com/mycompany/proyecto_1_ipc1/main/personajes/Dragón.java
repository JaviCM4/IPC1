 ...
 * @author DELL
 */
public class Dragón extends Personaje {
                    ...
                    if (seMovio) {              
                        ...
                        if (!hayObstaculos) {
                            ...
                            if (comando.equals("a")) {
                                ...
                                columna -= distancia;
                                System.out.println("    * El Dragón se Movio a la Izquierda");
                            } else if (comando.equals("d")) {
                                casillaActual = tablero[fila][columna + distancia];
                                tablero[fila][columna + distancia] = letra;
                                columna += distancia;
                                System.out.println("    * El Dragón se Movio a la Derecha");
                            } else if (comando.equals("w")) {
                                casillaActual = tablero[fila - distancia][columna];
                                tablero[fila - distancia][columna] = letra;
                                fila -= distancia;
                                System.out.println("    * El Dragón se Movio hacia Arriba");
                            } else if (comando.equals("s")) {
                                casillaActual = tablero[fila + distancia][columna];
                                tablero[fila + distancia][columna] = letra;
                                fila += distancia;
                                System.out.println("    * El Dragón se Movio hacia Abajo");
                            }     
                            System.out.println(ANSI_GREEN + "--------------------------------------------" + ANSI_RESET);
                            paso = true; 
                        } else {
    ...
    public void atacarPersonaje(String[][] tablero, Arbol[] arboles, Personaje[] enemigos, int x, int y, int indice) {
        ...
        if (random < porcentajeDeEfectividad) {
                    ...
                    if (arboles[j] != null) {
                        if (arboles[j].getFila() == filaAtacada && arboles[j].getColumna() == columnaAtacada) {
                            ...
                            if (arboles[j].getVida() == 0) {
                                ...
                                arboles[j] = null;
                                System.out.println("    * Dragón destruyó un Árbol");
                            } else {
                                System.out.println("    * Dragón hizó un daño de " + daño + " al Árbol pero sigue de Pie");
                            }
                            encontroAlgo = true;
                            break;
                        }
                    ...
                    if (enemigos[j] != null) {
                        if (enemigos[j].getFila() == filaAtacada && enemigos[j].getColumna() == columnaAtacada) {
                            ...
                            if (enemigos[j].obtenerVida() == 0) {
                                ...
                                System.out.println("    * Dragón dejo fuera de Combate a " + enemigos[j].mandarNombre()); 
                                enemigos[j] = null;
                            } else {
                                System.out.println("    * Dragón atacó a " + enemigos[j].mandarNombre() + " dejandoló con " + enemigos[j].obtenerVida() + " de vida");
                            }
                            encontroAlgo = true;
                            break;
                        }
            ...
            if (!encontroAlgo) {
                System.out.println("    * Dragón realizó su Ataque pero no habia Objetivos Cerca");
            }
            System.out.println(ANSI_GREEN + "-------------------------------------------------------------" + ANSI_RESET);
        } else {
            System.out.println(ANSI_GREEN + "   Información del Jugador:" + ANSI_RESET);
            System.out.println(ANSI_GREEN + "-------------------------------------------------------" + ANSI_RESET);
            System.out.println("    * El Dragón falló en el Ataque");
            System.out.println(ANSI_GREEN + "-------------------------------------------------------" + ANSI_RESET);
        }
    }
}
