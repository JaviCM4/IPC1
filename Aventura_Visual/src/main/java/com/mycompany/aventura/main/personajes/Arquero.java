...
public class Arquero extends Personaje {
    ...
    public void atacarPersonaje(ListaElementos<Casilla> tablero, ListaElementos<Arbol> arboles, ListaElementos<Personaje> personajesEnemigos, ListaElementos<String> casillasAnteriores, String direccion, ListaElementos<String> accionesEnemigos) {
        int columna_1 = columna, fila_1 = fila, contador = 0;
        boolean seMovio = true, encontroAlgo = false;

        Random random = new Random();
        int distancia = (random.nextInt(3) + 1);
        ...
        if (seMovio) {
            ...
            if (random_1 < porcentajeDeEfectividad) {
                int filaAtacada = fila_1;
                int columnaAtacada = columna_1;
                            ...
                            if (arboles.obtenerContenido(j).obtenerFila() == filaAtacada && arboles.obtenerContenido(j).obtenerColumna() == columnaAtacada) {
                                if (arboles.obtenerContenido(j).obtenerSiElArbolEstaActivo()) {
                                    ...
                                    if (arboles.obtenerContenido(j).obtenerVida() == 0) {
                                        ...
                                        arboles.eliminar(j);
                                        JOptionPane.showMessageDialog(null,"El Arquero (" + nombre + ") destruyó un Árbol","Información del Usuario",JOptionPane.INFORMATION_MESSAGE);
                                    } else {
                                        JOptionPane.showMessageDialog(null,"El Arquero (" + nombre + ") hizó un daño de " + daño + " a un Árbol","Información del Usuario",JOptionPane.INFORMATION_MESSAGE);
                                    }
                                    encontroAlgo = true;
                                    break;
                                }
                        ...
                        try {
                            if (personajesEnemigos.obtenerContenido(j).obtenerFilaDelJugador() == filaAtacada && personajesEnemigos.obtenerContenido(j).obtenerColumnaDelJugador() == columnaAtacada) {
                                ...
                                if (personajesEnemigos.obtenerContenido(j).obtenerVida() == 0) {
                                    ...
                                    JOptionPane.showMessageDialog(null,"El Arquero (" + nombre + ") dejó fuera de Combate a " + personajesEnemigos.obtenerContenido(j).obtenerNombre(),"Información del Usuario",JOptionPane.INFORMATION_MESSAGE);
                                    punteo = personajesEnemigos.obtenerContenido(j).obtenerPunteo();
                                    personajesEnemigos.eliminar(j);
                                    casillasAnteriores.eliminar(j);
                                } else {
                                    JOptionPane.showMessageDialog(null,"El Arquero (" + nombre + ") atacó a " + personajesEnemigos.obtenerContenido(j).obtenerNombre() + " dejandoló con " + personajesEnemigos.obtenerContenido(j).obtenerVida() + " de vida","Información del Usuario",JOptionPane.INFORMATION_MESSAGE);
                                }
                                encontroAlgo = true;
                            }
                        } catch (ListaElementosExcepcion ex) {
                ...
                if (!encontroAlgo) {
                    JOptionPane.showMessageDialog(null,"El Arquero (" + nombre + ") atacó pero no habia Objetivos Cerca","Información del Usuario",JOptionPane.INFORMATION_MESSAGE);
                }
                
                HiloDelAtaqueNormal iniciar = new HiloDelAtaqueNormal(tablero, (distancia - contador), fila, columna, direccion);
                iniciar.start();
            } else {
                JOptionPane.showMessageDialog(null,"El Arquero (" + nombre + ") falló el Ataque","Información del Usuario",JOptionPane.ERROR_MESSAGE);
            }
        } else {
